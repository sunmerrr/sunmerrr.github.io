...
*생각 - 마이크로 프론트엔드로 가고 싶은 생각은 항상 굴뚝 같지만 쉽지 않은 접근과 결정인 것 같다. 꼭 배포는 따로 하는 마이크로 프론트엔드가 아니더라도 프로젝트 내부에서 모듈화를 하도록 설계를 해보는 것도 좋을 것 같다.*

----
### 언제나, 누구에게나, 평등하게 빠른 웹
**결론: 기기 문제는 조금 어렵지만 캐싱, 프리로드 등의 방식으로 네트워크 변수로 인한 느려질 수 있는 웹을 방지 할 수 있음**
...
*[토스 페이먼츠 개발자 어쩌구(여기인듯)](https://developers.tosspayments.com/)에 방문하면 코드 샌드박스가 있어서 경험 가능!*
...
- 기억에 가장 많이 남았던 세션이 아니었을까 싶다. 나만을 위하는 사람에서 조직에 어우러지는 사람으로서 성장하는 것은 모두의 목표인 것 같다.
- '좋은 기술'이 '쓰이고 확산'되도록
...
*하지만 여전히 잘 만들지도, 쓰임세 있게 만들지도 못하는데.. 제대로된 엔지니어가 되려면 조금 멀었다는 생각도 든다..*
